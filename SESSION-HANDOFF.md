...
**Follow-up (deferred, cross-file — not done at close):** the N1b de-box waves created ~5 near-identical hairline-pill-group inline blocks (`TableControls`, `ProjectHealthCard`, `IdeasPage`, Dashboard tabs, `ManuscriptsPage`). Converge them on a shared `SegmentedToggle` (extract from the TableControls anatomy) — a clean reuse pass when someone next touches those toolbars. (No Hub `Docs/tech-debt-backlog.md` exists; recorded here.)
