...
>
> **DONE + LIVE (prod-verified, all pushed):**
...
> - **Phase 4** (2026-07-23) — backfill: `api/backfill-v102-daily-thought.sql` inserted **32 activity_entries rows**
>   into prod (16 `@hermes` roots `nick-ingraham` + 16 answer replies `claude-ai`; 9 `day` + 7 `task` conversations;
>   all `kind='comment'`). 🔴 **all 32 `visibility='author'` — verified post-apply** (no private exchange published).
>   codex `gpt-5.6-sol` = SHIP; test-DB smoke + prod dry-run + collision/NULL pre-flight + postflight all green.
...
**What shipped:**
- **#94 — project-feed lifecycle rows now NAME their task.** A task `completion` row rendered its bare body `"Completed"` with no subject (Nick: "what did i complete"). Frontend-only: the API already joined `task_title` into `handleGetProjectActivity`, and `ActivityEntryItem` already computed a task label + deep-link — but the `system|completion` early-return fired BEFORE that computation and handed `LifecycleActivityLine` no way to name its subject. Fixed at the CLASS (every task lifecycle body — created/status/assigned/completed — is written from the task's POV and was equally subject-less on a project feed), not just "completed". Renders `✓ Completed · <task name> — Nick`, name links `?openTask=`. Gated on `showTaskOriginBadge` (project feed only); a task's own feed stays bare, where the subject IS the page.
- **#95 — Projects list sorts by a real "last worked on" signal.** The sort/pinning was already built (`sortKey='activity'` default, pinned-first); **the data had no producer** — `Project.lastActivity` was read by 8 UI sites and assigned by nothing, so the sort silently fell back to `updated_at` and the "Xd ago" staleness chip **never rendered once**. `handleGetProjects` now derives `last_activity` = `MAX(activity_entries.created_at)` per project (project rows ∪ its tasks' rows = Nick's stated basis: "documenting updates" + "getting tasks done within them"), one constant-cost aggregate merged in memory. **The `seq_after` cursor branch (PB sync wire) is deliberately left byte-identical** — browser gets a derived display projection, replication gets the raw row (same split as the `tasks.project_id` slug projection). 74/84 projects now carry a real signal (vs 45 for the old `last_meaningful_movement` fallback).
- **Projects list row alignment (Nick, follow-on).** Columns were always fine (header + row share one grid, lefts matched to the pixel); the drift was VERTICAL and had two stacked causes: (1) the title cell was a TWO-line stack (title + `short_name`) so it alone set the grid track height, and every single-line value centered in that tall track — landing in the GAP between title and subtitle, 9px below the title it labels; (2) PI + Group were wrapped in bare `<div onClick>` blocks (there only to swallow the row click) which dropped the flex centering Status/Stage get, leaving them 2px lower still. **Root cause of both, plus the "dividers cut through the short name" report: the row is locked to 44px with 11px padding = a 22px content box, but the two-line title cell needed 36px** — content overflowed and the border-bottom (painted at the row edge) landed mid-subtitle. Nick's call: **keep 44px, drop `short_name` to the hover tip** → the title cell is ONE line, content clears the divider by 12px, and plain `align-items:center` lands all five columns on one pixel. That also REVERTED the interim `align-items:'start'` + two `minHeight:24` shims (they existed only to cope with the two-line cell).
- **Tooltip scope (Nick).** The title tip was on the `flex:1` wrapper (which spans the rest of the 817px column to push trailing badges right), arming the whole empty row as a hover target. Moved onto the title TEXT; wrapper made `display:flex` so the title span shrink-wraps to its content. Hover zone 677px → 55px. ⚠️ The code carried a comment insisting the tip must live on the wrapper to dodge the CSS `.tip::after` clipping — **that reason died 2026-07-09 when tooltips moved to `TooltipLayer`** (body portal, `position:fixed`, escapes overflow). Comment replaced; don't "restore" it.

**⚠️ Verification gotcha that cost 3 false alarms this session — READ BEFORE DEBUGGING A "DIDN'T SHIP" FIX.** Three times a deployed fix measured as absent in the browser because the tab held a **stale hashed JS chunk**. Neither a plain reload, `caches.delete()`, nor a `?cb=` query dislodged it (no service worker involved — plain HTTP document cache). Once it even presented as a convincing *layout* failure (old DOM structure + old computed styles). **The reliable check is to compare the live entry's chunk reference against the local build hash**, not to trust the rendered page:
```js
const html = await fetch('/portal/projects?x='+Date.now(),{cache:'no-store'}).then(r=>r.text());
const entry = html.match(/\/assets\/index-[A-Za-z0-9_-]+\.js/)[0];
const js = await fetch(entry,{cache:'no-store'}).then(r=>r.text());
[...new Set([...js.matchAll(/Projects-[A-Za-z0-9_-]+\.js/g)].map(m=>m[0]))]   // vs `ls dist/assets/Projects-*.js`
```
Then force a genuinely NEW document URL (`?bust=<unique>`) to reload it. Memory: `project_repo-environment-gotchas`.

**Env:** the git-bash `fork()` storm was heavy again (~20 blocked calls; build/deploy needed up to 4 retries). Retry wins — see `project_repo-environment-gotchas`.
...
`~/PB-State/data/shared/home-work-chat.md` (full diagnostic thread; HOME resolution post 16:08Z).

**Baselines confirmed:**
| | migration | contract | outbox unsent | local_modified | active dead-letter | active tasks | proj |
|---|---|---|---|---|---|---|---|
| **HOME** | 106 | 0.5.0 | 0 | 0 | 0 | 856 | 79 |
| **WORK** | 106 | 0.5.0 | 0 | 0 | 24 (inert) | 847 | 79 |
| **Hub (canonical)** | — | — | — | — | — | 858 | 79 |
snapshots: HOME `brain_2026-06-14_102213.db`, WORK `brain_2026-06-14_102827.db`.

**Gate 1 — dead-letters → ✅ RESOLVED.** WORK's 2 M5-relevant DLs are safe (`manual_merged_clean`, state-equivalent); its other 22 are inert pre-P4 kg residue. HOME = 0 active.

**Gate 2 — task-count delta → ✅ RESOLVED as a DIFF-TOOL ARTIFACT (3-seat panel UNANIMOUS + live proof).**
NOT duplicate-data, NOT a missing alias (the earlier "missing entity_aliases link / dup-identity" read was WRONG —
superseded). The ~19 hex/ULID pairs are the **DESIGNED forward-only steady state** of the 2026-04-29 hub-typed-ULIDs
cutover: pre-cutover Hub rows keep their 32-char hex id; brain.db keeps its typed `task_*` ULID; an ACTIVE
`entity_aliases` `hub_slug` alias bridges them; sync resolves THROUGH the alias on every hop (`ids.py resolve_entity`,
`sync/drivers/hub.py:1791`, `sync/hub_payload.py:303`). Our raw id-set diff bypassed the alias layer — the documented
anti-pattern at PB `rules-mistake-patterns.md:474`. **Proof (live):** the same diff resolved through `hub_slug`
collapses 22 Hub-only + 20 HOME-only → **2 + 1**, all trivial test/idea rows. Panel = mechanic + builder + codex
(independent, unbiased, unanimous); intent recovered = DELIBERATE (`sync_pull_peer_adopt` `b8f2c14f`, cutover decision
risk #3); frozen (closed after the 04-29 Hub mint flip `e2ad2d85`). Memory: `alias-aware-id-diffs-only`.

⚠️ **DO NOT** "reconcile" by `retire_local_duplicate` / dropping local ULIDs / retiring `hub_slug` aliases — all 3
seats flag that as the destructive I18/I19 class (kills the only bridge → orphan + re-duplicate). **There is NOTHING to clean.**

**M5 = FULLY UNBLOCKED. Phase 0 gate is GREEN** — the id divergence was never real; M5's substrate is clean.

**FRESH-SESSION NEXT ACTION:** proceed to M5 **Phase 1** (relay-gated) when Nick wants — Task 1 = additive migration
**107** (`description` column + backfill on BOTH machines via relay); Task 2 = Hub `pwaTask.notes` fallback drop.
Phase 2 (atomic flip) stays relay-gated after Phase 1's both-machine audit. (The ~3 test-task residuals + the
~2-row 858/856 gap are benign + separate — measure alias-aware if ever revisited.)
...
**M5 notes-privacy — SCOPED + SPECCED + codex-audited (NOT yet implemented).** Brainstormed with Nick → spec at `docs/superpowers/specs/2026-06-13-m5-notes-privacy-final-design.md` (codex AMEND verdict folded in as amendments). **Key reframe:** the timeline + PB marker-emit already shipped (activity_entries v77/v78, P2-B), so most of the old 2026-05-26 M5 plan is moot — remaining = sever the brain.db `notes`↔team `description` leak + split `notes`'s 3 conflated jobs (events→timeline ✅done, body→a clean editable `description` field, private→`@me`). `notes` retired as a concept; one body field. **Implementation = a FOCUSED next session** — needs the other machine online for relay-confirm + a brain.db snapshot; the rename lives in generated contract artifacts (`brain-db-schema-snapshot.json`, pb-schema) that must flip atomically (codex #3/#4). **Impl plan WRITTEN 2026-06-14** → `docs/superpowers/plans/2026-06-14-m5-notes-privacy-impl.md` (4 phases, relay-gated, 3-repo lockstep PB+pb-schema-submodule+Hub; grounded in re-grepped current code — schema_dsl SSOT flip + brain.db migration 083 + contract 0.5.0→0.6.0). Ready for Nick review; execution is relay-gated (Phase 0 baseline + Phase 2 flip need the other machine online).

**`fa5f20cd` shared SegmentedToggle (DONE at close + verified):** the N1b de-box waves had left ~4 near-identical hairline-pill-group inline blocks → extracted `src/components/ui/SegmentedToggle.tsx` (the ONE locked toggle anatomy: hairline group, active = accent TINT + text, never solid/tray; props accent teal|gold, size, uppercase, scrollable) and converged TableControls (canonical → all DataPage view toggles incl. Manuscripts follow), ProjectHealthCard, IdeasPage, Dashboard view tabs. Screenshot-verified (overview gold tabs / Manuscripts icons / Ideas) — no regression. **Use SegmentedToggle for any new view/mode toggle; don't re-mint the inline block.**

**`42103765` N1b de-box wave 5 — screenshot-audit find (deployed + visually verified):** ran a fresh desktop+mobile capture (`review/debox-audit-2026-06-13/`, gitignored) against the live preview with fake-auth. Confirmed waves 2–4 landed clean; surfaced ONE remaining solid-fill toggle the grep sweep missed — the Lab Overview view switcher (Overview/Projects/People/Deadlines), a `--surface-1` tray + solid `--stage-fill-analysis` active block (most obvious on mobile). Converged to the hairline-pill anatomy + gold TINT (`--gold-active`/`--gold-on-emphasis`, AA per Rule 42), neutralized the `.dark .dashboard-tabs !important` tray override. Re-captured = verified clean (gold tint pill). Audit tooling: `capture-n1-mobile-audit.spec.ts` now honors `CAPTURE_OUT_DIR` + covers Ideas/Digest; reusable via `CAPTURE_BASE_URL=<ungated-preview> npx playwright test --config=playwright.config.n1-audit.ts`.
> Minor pre-existing mobile nit seen in the audit (NOT mine, low-pri): the Today/Overview status-chip row ("All clear / 0 overdue / 0 this week") overflows the right edge on phones — "0 this week" clips. N1-class chip-row-overflow, left for a future mobile pass.

**`8d46414f` N1b de-box wave 4 — one-pill-language (deployed):** swept divergent active-toggle styles onto the tint anatomy (DensityToggle, IdeasPage view toggle, ActivityStream + ProjectDetail Note/Comment toggles — all off `--teal-solid`/`--surface-2`-tray/`--cream`-active onto `--teal-active` tint). Kept semantic fills (DoneBox/checkbox/progress) + the Digest gold status-tab idiom. **The N1b de-box sweep + the icon-discipline class are now both COMPREHENSIVELY DONE** (5 deploys this session). The queued style-sweep Hub tasks (task_01KTWD89… icon sweep, one-pill-language, etc.) are satisfied. **Remaining = subjective/infra only:** LabHealthScore greeting-chip scope (needs Nick's eye); the `python3`→`python` deploy-script fix (entangled with the d1-lint hook over pre-existing raw `wrangler d1` dev scripts — clean fix routes those 4 through `scripts/wrangler-d1`).

**`a6e00a32` icon class FULLY closed + icon-well de-box (deployed):** the static-name codemod only saw in-file lucide imports — dynamic prop-passed icons (`<Icon>`/`<IconComponent>` typed `LucideIcon`) were missed. New codemod (`scripts/icon-props-codemod-dynamic.cjs`) applied ICON_PROPS to 48 dynamic renders / 33 files; LinkChip + LinkifiedText (Icon from `classifyUrl()`, no direct lucide import) fixed manually. Re-run + grep = 0 remaining. Also executed the deferred JUDGMENT well de-box: BentoCard `--surface-2` header-icon well + StatsCard `--gold-active` mini-stat box dropped (2nd box layer per card), glyphs bumped 15→16/18 to recover weight. LabHealthScore greeting-chip scope still deferred (genuinely needs Nick's eye on whether the header band counts as "in-view").
> ⚠️ Identity gate (`predeploy:identity`) flaked once this session (transient prod-D1 read in the gate) — re-ran clean. If it FAILs, re-run before assuming a real drift.
...
- **`2294ba9e` N1b de-box wave 2** (deployed): Digest "For You" filter off `--teal-solid` fill → `--teal-active` tint; Manuscripts category tabs + stalled pill resting hairlines → transparent. MeetingDetail "selected-pills" reviewed = NOT a violation (legit action button), left as-is.
- **`e232394a` N1b de-box wave 3** (deployed): dashboard-card interiors — 7 box-in-box flattens (UpcomingCard banner+empty-state, ProactiveBriefCard suggested-focus, InsightsCard refresh→ghost, ProjectHealthCard view-toggle→TableControls anatomy, Pomodoro/WeeklyProgress tooltips→shadow). Code-audit by hub-frontend agent. Kept semantic chips/chart marks/avatar rings. **DEFERRED to Nick (JUDGMENT, app-wide look change):** BentoCard icon-well `--surface-2` bg + StatsCard mini-stat `--gold-active` icon bg (every card carries a 2nd bg layer for icon housing); LabHealthScore greeting-chip scope. Detail in design-system.md de-box status.
- **N1b de-box sweep is now effectively complete** through the named waves (drawers ✅ prior · toolbars ✅ w2 · dashboard interiors ✅ w3). Only the deferred JUDGMENT items remain — they need Nick's eye, not autonomous execution.
...
> ~~gardener maiden --apply artifact review (ran 22:40, home)~~ **CLOSED 2026-07-28 — the
> artifact is `gardener-2026-06-13T040439Z-apply.json`: it ran, examined 9 rows, changed
> nothing** (see the corrected note below the docket header), N1b remaining de-box waves ride
...
> **CLOSED 2026-07-28 — the maiden artifact EXISTS and is named below. The path above is wrong.**
> Runs live in `~/PB-State/data/shared/gardener/runs/` (Syncthing, on BOTH machines), not
> `data/gardener/`; the repo-relative path never existed and sent a later session hunting on the
> wrong disk (PB #916).
>
> The 2026-07-24 closure said "there is nothing to review." That was wrong, and the correction is
> the schedule: `schedule.json` runs `activity-gardener --apply` **daily at 22:40, home-only**.
> 22:40 CDT = ~03:40Z the NEXT day, which is the shape of every nightly filename. Read the
> timeline through that and it is legible:
>
> ```
> 06-11 07:07 + 07:09 CDT   two manual dry runs          (T120751Z, T120920Z — both empty)
> 06-11 22:40 CDT           planned maiden -> T034xZ 06-12   ABSENT
> 06-12 22:40 CDT           FIRST APPLY, written 23:04    -> gardener-2026-06-13T040439Z-apply.json
> 06-13 22:40 CDT           second apply (T034505Z 06-14), nightly thereafter
> ```
>
> **`gardener-2026-06-13T040439Z-apply.json` IS the maiden `--apply` artifact** (verified on work,
> 2026-07-28): `applied: true`, delta 14, **9 machine-origin rows admitted**, 5 left alone, LLM
> (haiku) called on all 9 and returned **0 collapse + 0 reclassify proposals**, 0 exact-dup
> deletions, 0 actions, 0 rollback SQL. So Nick's open item has a real answer: **it ran, examined
> 9 rows, and changed nothing** — the same answer as every night since. The maiden run did not
> vanish; it ran one night later than the 11th-written plan on line 644 predicted.
>
> Aggregate re-derived 2026-07-28 (40 files: 38 apply + 2 dry): the gardener has taken **0 actions
> in 38 apply runs**, 23 machine-origin rows admitted, none duplicates. (The prior "34 apply-mode
> runs" was a stale count — re-enumerate, do not copy it forward.) That is a quiet timeline, not a
> broken gate (`scripts/gardener/gate.py:57-67` excludes `pb_progress_note` deliberately — do not
> "fix" it).
>
