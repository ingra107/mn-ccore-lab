...
**Live deploy = `06cc7ebc` (Source `8d46414` = HEAD). Frontend-only — no schema/route/API change (still 74 tables / 240 routes / v82).** First deploy this session (`62d2896c`) also shipped the 2 backend commits that had sat undeployed 21h (`bc636726` i40 derive-email_link on A3 applyPatch, `2506ec35` artifacts batch key_link anti-pattern marker) + `e0c50dd` (artifacts auto-link, already live).
