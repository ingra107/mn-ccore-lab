...
**#123 — no way to hide finished projects.** The page filtered by category and not by status, so all 11 done projects of 92 sat in the list. A second pill group behind a hairline: Active (default) and All. Active means every project that is not done, so blocked and waiting stay in view — a different question from `isProjectActive()`, which is the narrower "in motion" test the health widgets use, so this added `isProjectDone()` rather than negating it. The gate runs before the category branches, so the counts, the pipeline columns and the list all describe the same population, and the strip says how many done rows are hidden.

**#122 — overlapping meetings were unreadable.** In an overlap cluster each meeting gets a ~200px column, and the row header's non-shrinkable children already exceeded it: 90px reserved for the time range, plus Join, Prep, the caret and the dismiss, plus five 12px gaps. The only flexible child is the title, `min-width: 0`, so it was squeezed to zero and the pills spilled into the next column. Measured on the built CSS at 3×200px — before: title width 0, last control 117px past the column edge; after: title 117px across two lines, last control 14px inside. The trailing controls are one group that takes its own line when the header wraps; the end time is hidden in overlap columns only, with the full range on hover.

**#124 — a meeting could not be talked to.** You could read a debrief and ask nothing about it. A meeting is now the fifth entity type on the one activity store, added the way `day` was, so threading, `@me` privacy, mentions and dismiss all come from `postActivityEntry` and the frontend reuses `ActivityThread`. `project_id` stays NULL: a meeting routinely tags several projects, so charging its conversation to one would be arbitrary and would move that project's health score for a discussion it never had.

An `@hermes` ask carries the meeting's agenda, notes, decisions, tasks and the path to Peripheral Brain's archived Zoom transcript in the prompt, because the deployed listener has no `meeting:` resolver and the fenced model cannot resolve an opaque `mtg-` id. Verified on prod: Hermes answered "straight from the transcript's closing minutes" and quoted what was said — and it did that on the OLD listener, which is also the proof that the new `meeting_comment` source type needed no cross-repo lockstep.

**Hermes answers render as markdown.** It writes numbered lists and bold unprompted and the feed showed the raw markers. `HermesResponse` renders through the `MarkdownView` the repo already had — whose own header named this gap. One caller, so it covers every Hermes surface. Human comments stay plain: people type asterisks without meaning bold.

**#121 — Hermes reads Peripheral Brain now, under a two-tier policy.** It always could (`Read` is in its tools, the CLI spawns in the repo); it was never told it may, where to look, or who it may repeat things to, and `requested_by` never reached the model so no policy could key on identity. Nick's asks are unrestricted; everyone else gets project-level material and a named deny list. An unattributed request falls to the restricted tier. The fence is the model following instructions — Nick's explicit call over a structural one — and sign-in is a `@umn.edu` policy plus auto-provisioning, so the audience is anyone with a University of Minnesota login.

**#119 — Nick's own asks get his Google Workspace tools.** Dismissed as a bug in September and built here. What Hermes may do is copied from the posture he already runs interactively: the whole `google_workspace` surface minus send and the destructive deletes. Three flags fail silently on this path and each cost a round of "no Gmail tool available" while the argv looked correct — a headless `--print` does not load servers from `~/.claude.json`, `--mcp-config` needs a file because inline JSON does not survive Windows argv quoting, and `--safe-mode` disables MCP servers outright. `--strict-mcp-config` is kept: without it the spawn also picked up the Slack connector, `slack_send_message` included. Nick's asks now pay the hook chain, 71s against ~5.5s; team asks keep the flag.

**A dead notification link, found by the simplify pass.** Four ladders in `activity-entry.ts` branch on entity type and #124 extended three of them, so an `@mention` in a meeting comment built `/projects?open=<meeting id>` — a link to nothing. The `day` entity had the same hole since it shipped. Both fixed; the config-object fix for the shape is #125.
