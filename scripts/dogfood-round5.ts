...
const MODAL_ACTIONS: { page: string; label: string; trigger: (p: Page) => Promise<void> }[] = [
  { page: '/portal/my-tasks', label: 'create-task-modal', trigger: async (p) => { await p.locator('button:has-text("New Task")').first().click() } },
  { page: '/portal/ideas', label: 'create-idea-modal', trigger: async (p) => { await p.keyboard.press('n') } },
  { page: '/portal/decisions', label: 'create-decision-modal', trigger: async (p) => { await p.keyboard.press('n') } },
  { page: '/portal/dashboard', label: 'cmdk-palette', trigger: async (p) => { await p.keyboard.press('Control+k') } },
  { page: '/portal/dashboard', label: 'shortcut-help', trigger: async (p) => { await p.keyboard.press('?') } },
  { page: '/portal/dashboard', label: 'bug-report-modal', trigger: async (p) => { await p.locator('button[aria-label*="Report" i], button:has-text("Report a Bug")').first().click() } },
  { page: '/portal/dashboard', label: 'quick-capture', trigger: async (p) => { await p.keyboard.press('Control+i') } },
]
