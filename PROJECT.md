...
domain: Research
tier: 2-Biweekly
next_action: WORKPLAN "NEXT SESSION" queue CLEARED + DEPLOYED (2026-06-05, deploy `7653955d` on `8cc00130`, both repos pushed, /api/version 200, API 732/732, journeys 6/6). (1) `tasks.project_id` slug↔id half-migration FIXED — Direction 1 (store typed `proj_*` PK, present SLUG at the read boundary) via dual+Codex+live-prod ground truth; resolver is one `COALESCE`-subquery chokepoint in `TASK_SELECT_COLS`; `?project=` slug→id; 5 slug-straggler rows backfilled in prod; decision doc/registry/CLAUDE.md rule + regression guard. ⚠️ Do NOT re-key frontend to p.id and do NOT run `p2_hub_rekey_apply.py` (parent rekey already converged; cross-machine PKs diverge so slug is the wire form). (2) `9a007fd1` Today fixes runtime-verified + deployed. (3) edit-more inline quick-edit shipped. (4) P6 responsive shipped (UX-9 tablet breakpoint deferred). (5) key_links sync = not a bug (data-entry gap). **Open (small):** DH-5 Nick-eyes Key Links on live (CF Access gated); edit-more/P6 post-deploy visual confirm; optional PB-project per-store PK divergence reconcile (non-urgent). See SESSION-HANDOFF.md + CHANGELOG.md.
