...
domain: Research
tier: 2-Biweekly
next_action: Add the authorship check to the featured-publications PUT now that the author_slugs backfill has landed (PB #1126) — see REFERENCE.md "PUT /api/team/:slug/featured-publications". In-app bug queue is EMPTY (2026-08-05 close — #111/#112 fixed, both GitHub issues closed, live `bb9af5ec`). Next passes, in order: ⭐ Cross-page consistency sweep (TOP item in PB `Projects/mn-ccore-lab-hub/hub-future-ideas.md`, carrying the accumulated `/simplify` deferrals — overloaded `showTaskOriginBadge` prop, shared `GRID_COLS` per table); **#109** retire the meeting-ref bridge (durable fix is in PB — make the meeting writers mint canonical `meetings.id`, then backfill, then delete `decorateMeetingRefs`); **#110** make the timeline gap px↔minute invariant structural (`minToPx`/`pxToMin` pair, then the per-gap scale). CORRECTED, do not re-derive: do NOT swap hand-rolled day math to `daysSince()` until `daysSince` itself delegates to `parseDbUtc` (it parses bare D1 stamps as local — a real behaviour change across many callers, needs its own pass). Session-handoff redesign is PART-DONE — `scripts/gen-session-header.mjs` + its plan `docs/superpowers/plans/2026-06-15-session-handoff-shrink-generate.md` are COMMITTED and tracked (the old "NOTHING COMMITTED YET" warning here was false for weeks); what remains is (1) re-run the generator clean, (2) apply the shrink, (3) wire `--check` into `.githooks/pre-commit` + CI (verified still unwired). Lower-priority: Today-cockpit IA consolidation (`docs/superpowers/plans/2026-06-09-today-cockpit-ia-consolidation-plan.md`).
