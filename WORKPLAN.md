...
**Round-6 Task-UI design audit is DONE: Today drag bug FIXED + P3–P6 ALL SHIPPED to commits** (`a231fea7`…`663043e5`; build + `tsc --noEmit` green throughout; drag verified 3/3 on the local journeys stack and re-verified green after P3–P6). **Batch is committed but NOT yet deployed — awaiting push + `npm run deploy:pages:gated` go.** Source handoff (kept for reference): `review/MN-CCORE Lab Hub Design System (5)/design/Handoff — Task UI Consistency Pass.md`. Guardrails honored (presentation-only; no routes/API/hooks/schema; additive surface-by-surface).
