...
export default function MetricCard({ icon: Icon, label, value, color, subtitle }: MetricCardProps) {
  return (
    <div className="rounded-xl border p-3 sm:p-4" style={{ borderColor: 'var(--border-light)' }}>
      ...
      <div className="text-lg sm:text-xl" style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, color: 'var(--ink)' }}>{value}</div>
      {subtitle && (
        <span className="text-[10px]" style={{ fontFamily: 'var(--font-sans)', color: 'var(--slate)', opacity: 0.5 }}>{subtitle}</span>
      )}
    </div>
  )
}
