    ...
    >
      <div className="flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: iconColor }}>
            {title}
          </span>
        </div>
        {linkTo && linkLabel && (
          <Link
            to={linkTo}
            style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--teal)', textDecoration: 'none', opacity: 0.6 }}
          >
            {linkLabel}
          </Link>
        )}
  ...
  return (
    <div className="flex items-start gap-2 py-1">
      ...
      <div className="flex-1 min-w-0">
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: isOverdue ? 'var(--maroon)' : isUrgent ? 'var(--gold)' : 'var(--slate)', opacity: 0.7 }}>
          ...
          <SidebarCard title="Completed" icon={CheckCircle2} iconColor="var(--teal)" linkTo="/portal/tasks" linkLabel="View all">
            {recentlyCompleted.slice(0, 5).map((t: any) => (
              <div key={t.id} className="flex items-center gap-2 py-0.5">
                ...
                <span className="truncate" style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--ink)', opacity: 0.6, textDecoration: 'line-through' }}>
        ...
        <SidebarCard title="This Week" icon={Flag} iconColor="var(--gold)" linkTo="/portal/deadlines" linkLabel="View all">
          ...
          {stats.overdue > 0 && (
            <div className="flex items-center gap-2 py-1">
              ...
              <Link to="/portal/tasks" style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--maroon)', textDecoration: 'none' }}>
          ...
          {staleProjects.length > 0 && (
            <div className="flex items-center gap-2 py-1">
              ...
              <Link to="/projects" style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--slate)', textDecoration: 'none', opacity: 0.7 }}>
          ...
          {deepWorkHours !== null && (
            <div className="flex items-center gap-2 py-1 mt-1" style={{ borderTop: '1px solid rgba(201,168,76,0.06)', paddingTop: 6 }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--teal)', opacity: 0.7 }}>
          ...
          {openCommitments.map((c: any) => (
            <div key={c.id} className="flex items-start gap-2 py-1">
              ...
              <div className="flex-1 min-w-0">
                ...
                {c.due_date && (
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: c.due_date <= today ? 'var(--maroon)' : 'var(--slate)', opacity: 0.6 }}>
                    Due {c.due_date}
                  </span>
                )}
              </div>
