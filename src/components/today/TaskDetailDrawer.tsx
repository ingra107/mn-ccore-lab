...
import { ProjectInlineGhostSelect, DueInlineSelect } from '../tasks/detail/FieldControls'
...
export function TaskDetailDrawer({ task, project, state }: { task: TaskRow; project: { name: string; slug: string } | null; state: TodayStateApi }) {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} style={{ padding: '14px 16px 16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', flexWrap: 'wrap', columnGap: 6, rowGap: 4 }} onClick={(e) => e.stopPropagation()}>
        <GhostSelect
          aria-label="Status"
          value={task.status}
