...
export interface TaskRow {
  ...
  title: string
  /** Curated concise title (brain.db `short_title`, schema v47; generated by PB
   *  `generate_short_titles.py` for tasks whose title > 45 chars). Synced to D1 and
   *  returned by /api/tasks. Prefer over `title` for row display — full title stays
   *  available on hover + in the expanded drawer. A complete short title is not a
   *  truncation, so this is compatible with the non-truncating-title rule (Rule 68). */
