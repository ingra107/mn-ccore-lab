...
**1. DONE Ledger**
- **M3 P1 drift-closure largely DISSOLVED on the authoritative re-run (2026-05-25) — the real win was unblinding the detectors.** Audit-first (read-only /investigate-equivalent via builder) found the **drift invariants I5-I10 were silently BLIND**: `audit_runner.py:228` built an unauthenticated `httpx.Client` → Hub 401 → `invariants.py:hub_fetch_*` swallowed it identically to a network outage → every Hub-dependent contract skipped as "unreachable" since ~2026-05-01. **P0 fix (PB main `88bc1f4d`):** authed client (mirrors sync driver Bearer pattern) + new fail-loud `HUB_AUTH` invariant (CRITICAL on 401/missing-key, never silent-skip) + regression test. With detectors live, I5/I6/I7/I8/I10 all PASS — the ad-hoc-audit "26 tombstone gaps + 1 dup + 205 aliases" were artifacts of queries following RETIRED aliases the live-only resolver correctly ignores (same phantom shape as M1; conservative + audit-first prevented a phantom 200+-row Hub mutation). **P4 fix (PB main `46584ba0`):** generate-today staleness — `generate_today.ps1` skip-guard checked only the date field, not file age, so an interactive date-stamp made the 03:15 regen no-op (broke 05-24/05-25); now matches the Python generator's 4h freshness window + 6 `query.py` selectors hardened with canonical `'done'` exclusion. TODAY.md regenerated clean (42→22 tasks). Residual: I40 (4 email_links, scheduled janitor backfills), I20 (home self-converges), v55/`group_override` sync-correct-but-unexercised (revisit at M4). `WORKPLAN.md:13-18`
...
- **M4 P2 Task Workflow Surface is shipped + live (2026-05-26):** INFRA-7 v55 workflow fields (`waiting_on`/`promised_to`/`promise_date`/`next_checkin_date`) now surfaced — editor group in the task detail drawer (`FieldControls.tsx`/`TaskDetailDrawer.tsx`), compact badges on `TaskRow.tsx`, MyTasks "Waiting On" filter reads the real field (heuristic fallback kept); steer = distinct/freeform, separate from HandoffSection. D9 commitments `to_slug` added end-to-end (jsonl-canonical, additive D1 mirror column — NOT a promote/migrate): PB `commitments.py`+`aux_writers.py` (slugify fallback), Hub `schema-v69-commitments-to-slug.sql`, `handleCommitments`/`handleCreateCommitment`, `useCommitments`, person-avatar render. Deployed: remote D1 ALTER + Worker (`75809e4f`) + Pages (`06af842e`). Verified live: column present, GET returns `to_slug`, synthetic POST→201→exact-slug GET match→cleaned up. Commits PB `77d74578`, Hub `8d9a72c7`+`a3ff9004`. Plan: PB `Scratch/plans/2026-05-25-m4-task-workflow-surface.md`; decision: PB `Context/Decisions/2026-05-25-d9-commitments-to-slug.md`. `WORKPLAN.md:19-22`, `WORKPLAN.md:187-191`
...
M4 — P2 Task Workflow Surface ✅ DONE 2026-05-26 (see DONE ledger)
