...
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. **Schema version = the highest-numbered `api/schema-v*.sql`** (v102 on 2026-07-23), enforced by `python scripts/check-schema-versions.py` — which also gates commits, so the FILES are the source of truth and this line is only a hint. ⚠️ This row previously enumerated v75–v82 by hand and sat **18 migrations stale**, which nearly caused a new migration to be misnumbered on top of an existing one. Do not re-add a hand-maintained changelog here; add the migration file and let the checker track it. Recent: v100 = `activity_entries.parent_id` (threaded replies, #98); v101 = public-artifact CSP canary (#531); v102 = `activity_entries.hidden_at`/`hidden_by` (dismiss/hide, Hermes wave — Rule 79). |
