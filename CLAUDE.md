...
| Testing | Playwright 1.59 (4 suites) + Vitest 4.1 (`npm run test:api` is the canonical count — 848 as of 2026-06-11; counts drift, trust the run) |
...
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v82 (v75 = `tasks.planned_for/plan_slot/plan_rank`; v76 = `bug_reports`; v77 = `activity_entries` unified timeline; v78 = legacy message-table drops; v79 = `artifacts` + `artifact_versions`; v80 = retro parity for the formerly-unnumbered Slice-D `project_dependencies` rekey + daily_plans drop; v81 = `entity_seen` per-viewer seen tracking — new-activity teal ● signal vs NEW-assignment gold, `api/routes/seen.ts`, fail-soft pre-migration; v82 = `ai_requests.input_tokens/output_tokens` — Hermes per-generation usage, Hub-only). |
...
| Deploy mode | **TWO surfaces, one repo — deploy each deliberately.** (1) `pages.dev` = production site (frontend + `/api/*` fetch): manual via **`npm run deploy:pages:gated`** ONLY — no CI auto-deploy; its bindings live in the Pages project itself (dashboard — verified via `wrangler pages download config`, 2026-07-06), NOT in wrangler.toml. (2) The standalone `mn-ccore-lab-api` worker = the **CRON engine** (calendar poller / pulse / digest — Pages cannot run scheduled events): deploy via **`python scripts/wrangler_d1.py deploy --env production`**. wrangler.toml's top level is deliberately INERT (#500 post-mortem 2026-07-06) — prod bindings + crons live under `[env.production]` only, so `--name`/ad-hoc copies are born powerless; the wrapper blocks env-less deploys. ⚠️ History: #137 (2026-06-21) deleted this worker as "zero consumers" — a negative-space miss (the crons WERE its consumers; the orphaned api-test twin masked the breakage until the 2026-07-06 calendar outage). It was resurrected 2026-06-23. Do not re-retire without `substrate-swap`. Verify live commit: `wrangler pages deployment list --project-name mn-ccore-lab` (Source col); worker: `wrangler deployments list`. |
