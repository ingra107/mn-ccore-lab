...
| Testing | Playwright 1.59 (4 suites) + Vitest 4.1 (`npm run test:api` is the canonical count — 846 as of 2026-06-11; counts drift, trust the run) |
| Data | TanStack Query v5 + Cloudflare D1 (74 tables, sqlite_master excl. internal; 240 registered routes via Hono — pinned by the route-contract test) + Recharts |
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v81 (v75 = `tasks.planned_for/plan_slot/plan_rank`; v76 = `bug_reports`; v77 = `activity_entries` unified timeline; v78 = legacy message-table drops; v79 = `artifacts` + `artifact_versions`; v80 = retro parity for the formerly-unnumbered Slice-D `project_dependencies` rekey + daily_plans drop; v81 = `entity_seen` per-viewer seen tracking — new-activity teal ● signal vs NEW-assignment gold, `api/routes/seen.ts`, fail-soft pre-migration). |
...
74. **Icon discipline (Nick 2026-06-11: "pixely ≠ premium").** Icons at ≤20px use `strokeWidth={1.5} absoluteStrokeWidth` (lucide's default 2-on-24 scales fuzzy) and contained glyphs (`SquareCheck`, not overflowing `CheckSquare`). Pattern: `ICON_PROPS` const in `Sidebar.tsx`. Site-wide sweep beyond the sidebar is a queued task — apply on any surface you touch.
