...
24. **`actorSlug(email)` is a LUT, not a derive.** `EMAIL_PREFIX_TO_SLUG` in `shared/emailSlug.ts` maps email-prefix → canonical team slug. New members post-Phase-39 are auto-provisioned via `ensureTeamMember()` on first login — manual 3-step provisioning only needed for custom slug or non-@umn.edu members.
...
34. **Email prefix → slug via `emailToSlug`, never raw `split('@')[0]`.** `src/lib/emailSlug.ts` exports `emailToSlug(email)`, and `api/helpers.ts`'s `actorSlug()` resolves the same way. Both read `EMAIL_PREFIX_TO_SLUG` from `shared/emailSlug.ts` — **one file, one edit** to add a team member. It used to be two hand-mirrored copies, and #906 is what a missed lockstep looked like: the edit button hid while the API still accepted the write.
...
| A `position: fixed` panel clipped inside a dashboard widget | `DashboardGrid` renders every widget inside a react-grid-layout item carrying a CSS **transform** (`useCSSTransforms`), and a transformed ancestor becomes the containing block for `position: fixed` — so a panel/backdrop mounted inside a widget is clipped into the card instead of covering the viewport. Render it through `createPortal(..., document.body)`. Ref: `ActionBoardCard.tsx` (#114). |
...
| A timeline gap rendered at a non-linear height | **A gap's height MUST stay `minToPx(minutes)`.** Its interior is a coordinate system, not whitespace: six sites convert pointer pixels to minutes through `pxToMin` — list-drop (`TodayDndContext`), block move + resize (`useTaskBlockDrag`, `useTaskBlockGesture`), block placement (`packTaskBlocks`), drop preview + in-unit now-line (`TimelineGrid`). Render a gap at any other height and **a task dropped near the bottom of a long gap is SAVED AT THE WRONG TIME.** Confirmed empirically 2026-08-03 (sub-linear gap compression tried, measured, reverted). **Since 2026-08-18 (`a738ec5c`) the guard is STRUCTURAL, not a comment:** `PX_PER_MIN` is module-private and the six sites share the exact-inverse pair `minToPx`/`pxToMin` (`timelineModel.ts`), so importing the bare ratio fails to compile (TS2459). Compressing gaps is still the right way to shorten the timeline — it needs a PER-GAP scale threaded through all six sites, which is PB backlog **#1595** (GitHub #110 closed pointing there). |
