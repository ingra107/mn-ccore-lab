...
| Data | TanStack Query v5 + Cloudflare D1 (76 tables, sqlite_master excl. internal; 238 registered routes via Hono) + Recharts |
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v77 (v75 = `tasks.planned_for/plan_slot/plan_rank`; v76 = `bug_reports`; v77 = `activity_entries` unified timeline; + Slice-D `project_dependencies` DROP+recreate DDL, unnumbered). |
...
63e. **Entity deep-links: every surface consumes `?open=`/`?openTask=` via `useOpenParam`** (`src/hooks/useOpenParam.ts`) — fires once when data is ready, then strips the param. Generators (search, ⌘K, copy-link, context menu) rely on it; when adding a new detail-capable page, add the consumer.
