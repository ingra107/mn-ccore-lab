...
| Testing | Playwright 1.59 (4 suites) + Vitest 4.1. **Never quote a test count from this file — run `npm run test:api`.** (1169 on 2026-07-22, and every number previously written here went stale.) |
| Data | TanStack Query v5 + Cloudflare D1 + Recharts. **Table + route counts are DERIVED, not documented** — see the row below. |
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. **Schema version = the highest-numbered `api/schema-v*.sql`** (v101 on 2026-07-22), enforced by `python scripts/check-schema-versions.py` — which also gates commits, so the FILES are the source of truth and this line is only a hint. ⚠️ This row previously enumerated v75–v82 by hand and sat **18 migrations stale**, which nearly caused a new migration to be misnumbered on top of an existing one. Do not re-add a hand-maintained changelog here; add the migration file and let the checker track it. Recent: v100 = `activity_entries.parent_id` (threaded replies, #98); v101 = public-artifact CSP canary (#531). |
| Counting things | Don't trust prose for counts — derive them: **tables** `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '_cf_%'` via `scripts/wrangler-d1` (75 on 2026-07-22); **routes** = the pin in `api/routes/route-contract.generated.test.ts` (254 on 2026-07-22), which fails the build when it drifts; **tests** = the run. |
