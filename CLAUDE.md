...
- **Current HEAD:** `3c6d20a` on `main`, pushed (CD round-5 batch 4).
- **Current deploy:** `7077314e.mn-ccore-lab.pages.dev`.
...
| Current deploy | `7077314e.mn-ccore-lab.pages.dev` (2026-04-23 night, CD round-5 batch 4; HEAD `3c6d20a`) |
...
53. **DD-1 mode toggle (MyTasks pilot) + DD-2 saved views (v1 localStorage).** MyTasks reads `?mode=now|data` + `?filter=<quickFilter>` from URL; state round-trips through URL so saved views can capture it. `useSavedViews(page)` stores `SavedView[]` under LS key `mnccore.savedViews.v1.<page>` (25-view cap). `<SavedViewsMenu page="..." currentQuery={searchParams.toString()} onApply={...}>` renders the pill + dropdown. Don't roll out Now/Data to other list surfaces without first defining a Now equivalent per page (Deadlines/Grants/etc. have none). v2 roadmap: D1-backed saved views table + cross-surface 14-entity merge stream + sidebar pins (DD-2 full parity).
