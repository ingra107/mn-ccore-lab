...
| Deploy mode | **Manual via `wrangler pages deploy` ONLY — no auto-deploy** (no Pages CI workflow exists; verified 2026-05-22 — pushed commits did NOT trigger a deployment). `pages.dev` = production (serves frontend + `/api/*`); use **`npm run deploy:pages:gated`** (or `pwsh scripts/deploy-all.ps1`). The standalone `mn-ccore-lab-api` worker (`wrangler deploy` / the old `deploy:gated` script) was **RETIRED + deleted 2026-06-21 (#137)** — zero consumers; do NOT redeploy it. Verify the live commit: `wrangler pages deployment list --project-name mn-ccore-lab` (Source col). |
