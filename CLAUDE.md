...
| Data | TanStack Query v5 + Cloudflare D1 (73 tables, sqlite_master excl. internal; 238 registered routes via Hono — pinned by the route-contract test) + Recharts |
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v80 (v75 = `tasks.planned_for/plan_slot/plan_rank`; v76 = `bug_reports`; v77 = `activity_entries` unified timeline; v78 = legacy message-table drops; v79 = `artifacts` + `artifact_versions`; v80 = retro parity for the formerly-unnumbered Slice-D `project_dependencies` rekey + daily_plans drop). |
...
51. **Search covers 15 entity types — extend on future entity adds.** `api/routes/search.ts` queries 15 tables in parallel (artifacts added 2026-06-11). When adding a new entity: add SELECT, push block, `TYPE_PRIORITY` entry, and `typeConfig` in `src/pages/portal/SearchPage.tsx`.
