...
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v76 (v75 = `tasks.planned_for/plan_slot/plan_rank`; v76 = `bug_reports`; + Slice-D `project_dependencies` DROP+recreate DDL, unnumbered). |
...
60. **MyTasks view picker far-left of filter row, persisted in `localStorage.mt_view`.** Three views share ONE toolbar. List view uses right-side drawer (cursor-stable j/k nav); Columns and Lanes use inline expand. Source: `src/pages/MyTasks/index.tsx`.
