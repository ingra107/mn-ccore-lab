...
| D1 database (prod) | `b8453e9b-7c5f-4029-b07d-dd89c05d00cf` (ENAM), binding: `DB`. Schema v70. |
...
- **`tasks.project_id` (+ all project-FK columns) STORE the typed `proj_*` PK; the read API PRESENTS the project SLUG.** Resolution is ONE chokepoint — a `COALESCE((SELECT p.slug FROM projects p WHERE p.id = t.project_id), t.project_id) AS project_id` subquery embedded in `TASK_SELECT_COLS` (`api/lib/task-cols.ts`), so no task-read endpoint can leak the typed PK; `?project=` resolves slug→id; `meetings.ts` aliases `tasks t` (do NOT reintroduce the `.replace(/\bt\./g,'')` strip — it corrupts the subquery). Storage=typed (stable, rename-proof FK), wire=slug — the documented layering (master plan `2026-05-26-codex-pb-simplification-teardown.md:46`: "slugs are Hub-owned display/routing fields"), every read consumer is slug-keyed, and cross-store typed-PK convergence is operationally fragile (Hub D1 `kg_entities`/`kg_relations` still hold slug-format project keys; the local kg re-key never propagated to Hub). Frontend keys project lookups by slug (`projectsByPid` is slug-keyed despite its name); PB pull reads the slug. Internal mutation paths (`applyInsert`/`advanceProjectMovement`/cascade) use the **stored typed PK**. Was a P2 `aa85c71b` half-migration (write flipped to typed PK, reads never migrated → ~20 tasks rendered unlinked + sync silently broken); fixed 2026-06-05. ⚠️ Do NOT "fix" `projectsByPid` to key by `p.id`, and do NOT run `scripts/p2_hub_rekey_apply.py` as the answer (parent-table rekey already converged in prod). Decision: PB `Context/Decisions/2026-06-05-tasks-project-id-store-typed-present-slug.md`.
