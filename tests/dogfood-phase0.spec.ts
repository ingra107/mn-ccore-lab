    ...
    test(`${path} — loads without console errors`, async ({ page }) => {
      ...
      page.on('console', (msg) => {
        if (msg.type() === 'error') {
          ...
          // Cloudflare Access sign-in interstitial injects its own logo as a data: URI in
          // an <img src="data:..."> which trips CF's own CSP on the sign-in page. This is
          // Cloudflare's own bug in their interstitial — not our code. Our react-resizable
          // data: URI was fixed (b4a95a66) by vendoring the CSS without the data: URI.
          // Filter only the CF interstitial logo (viewBox 0 0 62 50, URL-encoded SVG).
          if (/Loading the image 'data:image\/svg\+xml.*viewBox.*62.*50.*violates.*Content Security Policy/i.test(text)) return
